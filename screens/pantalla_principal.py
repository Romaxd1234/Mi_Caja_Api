...
class VentanaPrincipal(Screen):
    ...
    def configurar_sesion(self, origen="patron", nombre=""):
    ...
    def on_pre_enter(self):
        ...
        if os.path.exists(config_path):
            ...
            if store_config.exists("actual"):
                ...
                if os.path.exists(ruta_tienda):
                    store_tienda = JsonStore(ruta_tienda)
                    if store_tienda.exists("tienda"):
                        datos_tienda = store_tienda.get("tienda")
                        self.nombre_tienda = datos_tienda.get("nombre", "Tienda").strip("*")
        ...
        # Si es empleado, mostrar su nombre
        ...
        # Botones principales
        self.boton_venta = Button(text="Venta", size_hint=(.2, .1), pos_hint={"x": 0.02, "top": 0.95})
        self.boton_venta.bind(on_release=self.abrir_venta)
        self.layout.add_widget(self.boton_venta)

        self.boton_buscar = Button(text="Venta Inventario", size_hint=(.2, .1), pos_hint={"x": 0.02, "top": 0.82})
        self.boton_buscar.bind(on_release=self.abrir_venta_con_inventario)
        self.layout.add_widget(self.boton_buscar)

        self.boton_inventario = Button(text="Inventario", size_hint=(.2, .1), pos_hint={"x": 0.02, "top": 0.69})
        self.boton_inventario.bind(on_release=self.ir_a_inventario)
        self.layout.add_widget(self.boton_inventario)

        self.boton_corte = Button(text="Corte", size_hint=(.2, .1), pos_hint={"x": 0.02, "top": 0.56})
        self.boton_corte.bind(on_release=self.abrir_popup_corte)
        self.layout.add_widget(self.boton_corte)

        self.boton_registro_cortes = Button(text="Registro de Cortes", size_hint=(.2, .1), pos_hint={"x": 0.02, "top": 0.43})
        self.boton_registro_cortes.bind(on_release=self.ir_a_registro_cortes)
        self.layout.add_widget(self.boton_registro_cortes)

        if self.origen_login == "patron":
            self.boton_registro_semanal = Button(text="Registro Semanal", size_hint=(.2, .1), pos_hint={"x": 0.02, "top": 0.30})
            self.boton_registro_semanal.bind(on_release=self.ir_a_registro_semanal)
            self.layout.add_widget(self.boton_registro_semanal)

        # Solo mostrar botón Empleados si NO es empleado
        if self.origen_login != "empleado":
            self.boton_empleados = Button(text="Empleados", size_hint=(.2, .1), pos_hint={"x": 0.02, "y": 0.02})
            self.boton_empleados.bind(on_release=self.gestionar_empleados)
            self.layout.add_widget(self.boton_empleados)

        # --- NUEVO: Revisar préstamo pendiente al entrar ---
    ...
    def abrir_venta(self, instance):
        ...
        # Lista temporal de productos agregados (producto, precio)
        self.ventas_temporales = []

        # Área para mostrar productos agregados con scroll
        ...
        def registrar_venta(instance):
            ...
            # Cargamos las ventas anteriores (como lista)
            ...
            nueva_venta = {
                ...
                "productos": [
                    ...
                    for producto, precio in self.ventas_temporales
                ],
                "total": sum(precio for _, precio in self.ventas_temporales),
        ...
        def popup_cerrar_confirmacion(instance):
    ...
    def gestionar_empleados(self, instance):
        ...
        if os.path.exists("data/config.json"):
            ...
            if store_config.exists("actual"):
                ruta = store_config.get("actual")["archivo"]
                self.manager.get_screen("ventana_empleados").set_ruta_tienda(ruta)
